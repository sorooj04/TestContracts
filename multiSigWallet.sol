...
contract MultiSigWallet {
    ...
    constructor(address[] memory _validators, uint _approvalsRequired) {
        ...
        require(
            _approvalsRequired > 0 && _validators.length >= _approvalsRequired,
            "No owners exist"
        );
    ...
    function returnTransactionByIndex(uint _index)
        external
        view
        returns (
            address,
    ...
    function validate(uint _index) external {
        ...
        require(_index < transactions.length, "Incorrect index");
        require(
            transactions[_index].completed == false,
    ...
    function transactTransaction(uint _index) external {
        require(
            transactions[_index].approvalsCount >= approvalsRequired,
        ...
        (bool success, ) = transactions[_index].reciever.call{
            value: transactions[_index].amount
        }("");
